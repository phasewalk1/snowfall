use super::{Root, SnowfallPath, SnowfallPathVar};
use super::common::{PublicKey, PublicKeyVar, Gamma, GammaVar, ConstraintF, RootVar, CommitmentVar};
...
impl ConstraintSynthesizer<ConstraintF> for RegistrationCircuit {
    fn generate_constraints(self, cs: ConstraintSystemRef<ConstraintF>) -> ark_relations::r1cs::Result<()> {
        ...
        let comm = CommitmentVar::new_input(
            ark_relations::ns!(cs, "comm"),
            || Ok(self.comm.clone())
        )?;
        ...
        let path = SnowfallPathVar::new_witness(
            ark_relations::ns!(cs, "path"),
            || self.path.ok_or(SynthesisError::AssignmentMissing)
        )?;

        let gamma = GammaVar::new_witness(
            ark_relations::ns!(cs, "gamma"),
